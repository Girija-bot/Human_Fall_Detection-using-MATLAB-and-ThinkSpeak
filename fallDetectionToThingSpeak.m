function [] = fallDetectionToThingSpeak()
    ...
    while ~isDone(vid) % Process the entire video
        ...
        if ~isempty(box)
            ...
            % Detect fall
            fall_detected = detectFall(fgClose, mhimage);

            % Send data to ThingSpeak
            sendDataToThingSpeak(frameNo, fall_detected, mean(mhimage(:)));
        else
            % Output
            subplot(1, 3, 1);
            imshow(frame);
            ...
            % Send data to ThingSpeak
            sendDataToThingSpeak(frameNo, 0, mean(mhimage(:)));
        end
...
function fall_detected = detectFall(human_img, motion_img)
    % Placeholder logic: Detect large horizontal area in the motion image
    horizontal_threshold = 1000; % Adjust based on your scenario
    horizontal_motion = sum(motion_img, 1);
    fall_detected = any(horizontal_motion > horizontal_threshold);
end

function sendDataToThingSpeak(frameNo, detectionState, speed)
    writeAPIKey = 'RM41JJ3PW1IF0CUB';
    url = 'https://api.thingspeak.com/update';
    data = struct('api_key', writeAPIKey, 'field1', frameNo, 'field2', detectionState, 'field3', speed);
    webwrite(url, data);
end
